-- | A pretty printer for TeX documents.
module Lhs2TeX.Document where
...
data Doc =
  ...
  | Text String
  | Doc :^: Doc
  | Embedded String  -- embedded TeX text, e.g. in comments
  | Sub String [Doc] -- for subst-replacements
  deriving (Eq, Show)
...
-- The remaining definitions are abbreviations for
